...
// loop que recebe novas coneções no listening socked, lida com request quando coneção é aceite
void worker(int listenfd, int i) {
    while (1) {
...
// criar array para armazenar 10 pid's de 10 processos filho
pid_t childPids[NUM_WORKERS] = {0};
int listenfd_global;

void sigchldHandler(int sig) {
    for (int i = 0; i < NUM_WORKERS; i++) {
        int status;
        pid_t pid = waitpid(childPids[i], &status, WNOHANG);
        ...
        } else if (pid == childPids[i]) {
            // child terminated, create a new one
            childPids[i] = fork();
            if (childPids[i] == 0) {
                // in child
                worker(listenfd_global, i);
                exit(0);
            }
        }
    }
...
int main(int argc, char** argv) {
    ...
    // inicializa os processos worker
    ...
    while (1) {
        /*
            for (i = 0; i < NUM_WORKERS; i++) {
                int status;
                int pid = waitpid(childPid[i], &status, WNOHANG);

                if (pid < 0) {
                    logger(ERROR, "system call", "fork", 0);
                } else if (pid == 0) {
                    // filho continua a correr
                } else if (pid == childPid[i]) {
                    // processo filho terminou, vamos substituir por um novo
                    childPid[i] = fork();
                    if (childPid[i] < 0) {
                        logger(ERROR, "system call", "fork", 0);
        ...
        sleep(1);
        //whatever you write here it shows that execution or connection is closed
    }
}
